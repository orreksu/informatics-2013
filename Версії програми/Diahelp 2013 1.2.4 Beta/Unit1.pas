...
Label16.Caption:=f.Strings[1]+' кг';
...
try
// добавление в список трех строк 
...
// безусловное уничтожение объекта
 Free;
end;
  ShowMessage('Ви новий користувач. Введіть необхідні для розрахунку дані! (кнопка "Редагувати")');
   end;
...
begin
 ShowMessage('Программа розрахунку компенсацыйних доз інсуліну'+#13#10+'Diahelp 2013  ver. 1.2.4.'+#13#10+'Автор: Літус О.О.'+#13#10+'Гімfpsz @Ghtcnb;@')
end;
...
begin
 ...
 if Edit1.Text='' then ShowMessage('Введіть рівень глюкози!')
 else GK:=strtofloat (Edit1.Text);
 if Edit3.Text='' then ShowMessage('Введіть рівень цільової глюкози!')
...
I:=k*XO;
V:=I+Dps;
if GK>10 then MessageDlg ('Вживання їжі не рекомендоване!', mtWarning, [mbOk], 0);
...
case MessageBox(Self.Handle,'Записати розраховані дані у файл?','',MB_YESNO + MB_ICONQUESTION+ MB_APPLMODAL) of
IDNO :
 begin
  end;
IDYES:
begin
if RadioButton3.Checked then DT:='Р'
 else if  RadioButton4.Checked then DT:='Д'
 else DT:='В';
...
begin
 ShowMessage('Программа розрахунку компенсаційних доз інсуліну'+#13#10+'Diahelp 2013  ver. 1.2.4.'+#13#10+'Автор: Літус О.О.'+#13#10
 +'Гімназія "Престиж"')
end;
